...
std::vector<std::string> WritingElementManager::getAvailableOptions(const std::string& category, int level) const {
    ...
    for (const auto& option : it->second) {
        if (category == "장르") {
            if (option == "판타지") filtered.push_back(option);
            else if (option == "공상과학" && level >= 2) filtered.push_back(option);
            else if (option == "아포칼립스" && level >= 5) filtered.push_back(option);
        }
        else if (category == "분위기") {
            if (option == "몽환") filtered.push_back(option);
            else if (option == "암울" && level >= 2) filtered.push_back(option);
            else if (option == "공포" && level >= 5) filtered.push_back(option);
            else if ((option == "기묘" || option == "무거움") && level >= 10) filtered.push_back(option);
        }
        else if (category == "분량") {
            if (option == "30") filtered.push_back(option);
            else if (option == "90" && level >= 2) filtered.push_back(option);
            else if (option == "120" && level >= 5) filtered.push_back(option);
        }
        else if (category == "옛지 요소") {
            if (option == "없음") filtered.push_back(option);
            else if (option == "반전" && level >= 5) filtered.push_back(option);
        }
        else if (category == "기타") {
            if (option == "없음") filtered.push_back(option);
            else if (option == "숨겨진 코드" && level >= 30) filtered.push_back(option);
            else if ((option == "고대 룬" || option == "운명의 반전") && level >= 50) filtered.push_back(option);
            else if ((option == "미래 예시" || option == "금단 기술") && level >= 80) filtered.push_back(option);
        }
    }
